...
UDP_IP = ""
UDP_PORT = 5005
LEADER_IP = "10.49.245.19" # might change frequently
...
def distance_to_speed(dist_cm):
    global lost_flag
    if dist_cm is None or dist_cm <= 5:
        return 0
    elif dist_cm >= 80:
...
try:
    while stop_flag:
        ...
        if dist_a is not None and dist_b is not None:
            ...
            if diff > ALIGN_THRESHOLD and diff < ALIGN_THRESHOLD_PRO:
                speed_a = min(base_speed + ALIGN_BOOST, 100)
                speed_b = base_speed
                
            elif diff > ALIGN_THRESHOLD_PRO: 
                speed_a = min(base_speed + ALIGN_BOOST + 10, 100)
                speed_b = base_speed

            elif diff < -ALIGN_THRESHOLD and diff > ALIGN_THRESHOLD_PRO:
                speed_b = min(base_speed + ALIGN_BOOST, 100)
                speed_a = base_speed
                
            elif diff < -ALIGN_THRESHOLD_PRO:
                speed_b = min(base_speed + ALIGN_BOOST + 10, 100)
                speed_a = base_speed                
